...
colors = [:magenta, :white, :red, :green, :blue, :yellow, :black, :cyan,  ]
...
# set(axis="normal", plotstyle="points", linewidth=2, pointsize = 1, color = "black", xlabel = "x", ylabel = "y ", grid="on", title = "")
    ...
    pl = scatterplot(vec(fens.xyz[i1, 1]), vec(fens.xyz[i1, 2]), color = colors[ixxxx], xlim = [0 L/2], ylim = [0 H/2])
    for ixxxx = 2:length(partitionnumbers)
        i1 = findall(x -> x == partitionnumbers[ixxxx], partitioning)
        scatterplot!(pl, vec(fens.xyz[i1, 1]), vec(fens.xyz[i1, 2]), color = colors[ixxxx])
    end
    display(pl)

# Render the  points as dots ("Points" in paraview)
    Points = FESetP1(reshape(collect(1:count(fens)), count(fens), 1))
    File = "node_partitioning_1.vtk"
    MeshExportModule.VTK.vtkexportmesh(File, fens, Points;
...
# set(axis="normal", plotstyle="points", linewidth=2, pointsize = 2, color = "black", xlabel = "x", ylabel = "y ", grid="on", title = "")
    ...
    ixxxx = 1
    i1 = findall(x -> x == partitionnumbers[ixxxx], partitioning)
    pl = scatterplot(vec(fens.xyz[i1, 1]), vec(fens.xyz[i1, 2]), color = colors[ixxxx])
    for ixxxx = 2:length(partitionnumbers)
        i1 = findall(x -> x == partitionnumbers[ixxxx], partitioning)
        scatterplot!(pl, vec(fens.xyz[i1, 1]), vec(fens.xyz[i1, 2]), color = colors[ixxxx])
    end
...
# Visualize partitioning
    Points = FESetP1(reshape(collect(1:count(fens)), count(fens), 1))
    MeshExportModule.VTK.vtkexportmesh("node_partitioning_4.vtk", fens, Points; scalars=[("Partition", vec(partitioning))])
    @async run(`"paraview.exe" $File`)
end
...
# Visualize partitioning
    for gp = partitionnumbers
        ...
        MeshExportModule.VTK.vtkexportmesh(File, fens, FESetP1(reshape(groupnodes, length(groupnodes), 1)))
    end
    File =  "mesh-1.vtk"
    MeshExportModule.VTK.vtkexportmesh(File, fens, fes1)
    File =  "mesh-2.vtk"
    MeshExportModule.VTK.vtkexportmesh(File, fens, fes2)
    @async run(`"paraview.exe" $File`)
end
...
# Visualize partitioning
    ...
    for i = 1:length(fesarr)
        File =  "mesh-$(i).vtk"
        MeshExportModule.VTK.vtkexportmesh(File, fens, fesarr[i])
    end
    File =  "mesh-1.vtk"
    @async run(`"paraview.exe" $File`)
end
